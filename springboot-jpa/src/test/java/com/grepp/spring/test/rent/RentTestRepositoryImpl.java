...
public class RentTestRepositoryImpl implements RentTestRepositoryCustom{
    ...
    public List<Rent> whereOr() {
        return queryFactory.select(rent)
            ...
            .where(rent.userId.eq("jpa")
                .or(rent.title.startsWith("디디")))
            .fetch();
    }

    @Override
    public List<Rent> join() { // fetch join
        return queryFactory.select(rent).from(rent)
    ...
    public List<RentDto> projections() {
        return queryFactory.select(Projections.fields(RentDto.class, rent.title, rent.id))
            .from(rent)
    ...
    public List<Tuple> tuple() {
        return queryFactory.select(rent.id, rent.title, rent.userId).from(rent).where(rent.userId.eq("test")).fetch();
    }
