...
public class BookTestRepositoryCustomImpl implements BookTestRepositoryCustom {
    ...
    public List<Book> orderBy() {
        return queryFactory.select(book).from(book).orderBy(book.amount.desc()).limit(3).fetch();
    }

    @Override
    public List<Tuple> groupBy() {
        return queryFactory.select(book.category, book.amount.max(), book.amount.avg(), book.amount.count()).from(book).groupBy(book.category).fetch();
    }
}
